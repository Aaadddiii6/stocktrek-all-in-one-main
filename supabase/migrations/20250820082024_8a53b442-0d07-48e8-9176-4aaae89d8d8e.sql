...
-- Additional security: Create a view for read-only access that masks sensitive data
CREATE OR REPLACE VIEW public.courier_tracking_summary AS
SELECT 
  id,
  sr_no,
  date,
  LEFT(name, 1) || '***' as masked_name,
  LEFT(address, 10) || '***' as masked_address,
  ...
  created_at,
  updated_at
FROM public.courier_tracking;

-- Enable RLS on the view
ALTER VIEW public.courier_tracking_summary SET (security_barrier = true);

-- Create policy for the summary view (viewers can see masked data)
DROP POLICY IF EXISTS "Authenticated users can view masked courier summary" ON public.courier_tracking_summary;
CREATE POLICY "Authenticated users can view masked courier summary" 
ON public.courier_tracking_summary 
FOR SELECT 
TO authenticated
USING (true);